    ...
    ValueType: InterpolationValue + Send + Sync,
{
    ...
    fn cox_de_boor(&self, point: IndexType, index: usize, degree: usize) -> ValueType {
        ...
        if self.knots[index + degree] != self.knots[index] {
            left_term = ((point - self.knots[index])
                / (self.knots[index + degree] - self.knots[index]))
                * self.cox_de_boor(point, index, degree - 1);
        }

        if self.knots[index + degree + 1] != self.knots[index + 1] {
            right_term = ((self.knots[index + degree + 1] - point)
                / (self.knots[index + degree + 1] - self.knots[index + 1]))
    ...
    ValueType: InterpolationValue + Send + Sync,
{
    ...
    fn add_point(&mut self, point: (IndexType, ValueType)) {
        ...
        self.control_points
            .insert(self.control_points.len(), point.1);
    }

    fn interpolate(&self, point: IndexType) -> Result<ValueType, RustQuantError> {
        if !(point.ge(&self.knots[self.degree])
            && point.le(&self.knots[self.knots.len() - self.degree - 1]))
        {
